...
investigate <- function(keyword, nCores = 12, plotdir = "~/Downloads/") {
    ##
    ##
    ...
    ## do plot
    ##
    out <- make_informative_plot(results, result_phylo, keyword, height_scale = 0.2, plotdir = plotdir)
    return(
        list(
            results = results,
            result_phylo = result_phylo,
            tip.label = out[["tip.label"]]
        )
    )
}
